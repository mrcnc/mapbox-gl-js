...
global.propertyValue = function (property) {
    if (isDataDriven(property)) {
        return `new DataDrivenProperty(${runtimeType(property)}, ${defaultValue(property)})`;
    } else if (/-pattern$/.test(property.name) || property.name === 'line-dasharray') {
        return `new CrossFadedProperty(${runtimeType(property)})`;
    } else if (property.name === 'heatmap-color') {
        return `new HeatmapColorProperty(${defaultValue(property)})`;
    } else {
        return `new DataConstantProperty(${runtimeType(property)}, ${defaultValue(property)})`;
    }
};
