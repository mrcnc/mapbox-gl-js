...
class StyleLayer extends Evented {
    ...
    constructor(layer: LayerSpecification, properties: {layout?: any, paint: any}) {
        ...
        for (const paintName in layer.paint) {
            this.setPaintProperty(paintName, layer.paint[paintName], {validate: false});
        }
    ...
    getPaintProperty(name: string) {
        if (util.endsWith(name, TRANSITION_SUFFIX)) {
            return this._transitionablePaint.getTransition(name);
        } else {
    ...
    setPaintProperty(name: string, value: mixed, options: {validate: boolean}) {
        ...
        if (util.endsWith(name, TRANSITION_SUFFIX)) {
            this._transitionablePaint.setTransition(name, value);
        } else {
