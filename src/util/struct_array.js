...
const createEmplaceBack = browser.supportsEval ?
    createEmplaceBackWithEval :
    createEmplaceBackWithoutEval;
const createIndexedMemberComponentGetter = browser.supportsEval ?
    createIndexedMemberComponentGetterWithEval :
    createIndexedMemberComponentGetterWithoutEval;
const createAccessors = browser.supportsEval ?
...
function createEmplaceBackWithoutEval(members, bytesPerElement): Function {
    return function () {
        ...
        for (const member of members) {
            const size = sizeOf(member.type);
            const offset = i * Math.floor(bytesPerElement / size);

            for (let c = 0; c < member.components; c++) {
                const index = offset + Math.floor(member.offset / size + c);
                this[getArrayViewName(member.type)][index] = arguments[argIndex++];
            }
        }
